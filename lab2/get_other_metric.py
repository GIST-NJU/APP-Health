...
from ReviewRelated import get_font_num_list,get_date_list,get_date_reviewnum_dict
import datetime
import csv
import re
import os
...
def get_release_num(app_name):
    app_release_path='release/{}_all_releases_1.csv'.format(app_name)
...
def get_release_date(app_name):
    ...
    #改了一下日期显示
    result_file = 'release/{}_all_releases_1.csv'.format(app_name)
...
def get_release_date_and_labeled_reviews(app_name):
    #start_release_num = get_release_num(app_name)
...
def get_month_date_and_labeled_reviews(app_name):
    app_month_path = 'release/month.csv'
    months = read_csv(app_month_path)
    ...
    labeled_reviews = read_txt('review_predicted/{}_review_pred_v2.txt'.format(app_name))  # 带标记的评论
    #print(release_date_list, labeled_reviews)
    return month_date_list, labeled_reviews

def get_reviews_num(app):
    result_file = 'month_score/{}_all_month_reviews_num.csv'.format(app)
    result = open(result_file, 'w', encoding='utf-8', newline='')
    csv_writer = csv.writer(result)
    lines = 0
    # 15个关注点均分
    # 去均分
    df = pd.read_csv('raw_month_score/{}_all_month_score.csv'.format(app), header=None,
    names=["other", "feature assessment", "feature bug", "gui", "guide", "compatibility",
    "performance", "security", "account", "content", "ads", "feedback", "competitive",
    "request", "release", "all"])
    df = df.drop(['all'], axis=1)
    df.to_csv('raw_month_score/{}_all_month_score.csv'.format(app), header=None, encoding='utf-8', index=False)
    raw_files = read_csv('raw_month_score/{}_all_month_score.csv'.format(app))

    # 预测原始评论(得到评论个数）
    date_num = 0
    for score in raw_files:
        num = 0
        date_list, raw_reviews = get_month_date_and_labeled_reviews(app)

        for labeled_review in raw_reviews:
            labeled_review = labeled_review.strip('\n').split('-*-')
            # 评论的标签列表
            review_label_list = labeled_review[review_label_index].split('-')
            # 评论的内容
            ...
            review_date = labeled_review[review_date_index]
            review_date = datetime.datetime.strptime(review_date[0:-3], date_str1)
            if review_date <= date_list[date_num] and review_date > date_list[date_num + 1]:
                num += 1
                # latest_releases_reviews_list[int(label)].append(labeled_review)
            else:
                continue
...
def get_release_num(app):
    result_file = 'month_score/{}_all_month_score_rn.csv'.format(app)
    result = open(result_file, 'w', encoding='utf-8', newline='')
    csv_writer = csv.writer(result)
    lines = 0
    # 15个关注点均分
    raw_files = read_csv('month_score/{}_all_month_reviews_num.csv'.format(app))
    releases = read_csv('release/{}_all_releases_1.csv'.format(app))
    # 版本发布次数
    date_num = 0
    print(app)
    for score in raw_files:
        ...
        for release in releases:
            ...
            release_date = release[1].replace('/', '-')
            release_date = datetime.datetime.strptime(release_date, date_str)

            if release_date <= date_list[date_num] and release_date > date_list[date_num + 1]:
                num += 1
                # latest_releases_reviews_list[int(label)].append(labeled_review)
            else:
                continue
...
def get_release_front_num(app):
    result_file = 'month_score/{}_all_month_score_rf.csv'.format(app)
    result = open(result_file, 'w', encoding='utf-8', newline='')
    csv_writer = csv.writer(result)
    lines = 0
    # 15个关注点均分
    raw_files = read_csv('month_score/{}_all_month_score_rn.csv'.format(app))
    releases = read_csv('release/{}_all_releases_1.csv'.format(app))
    # 预测版本发布次数
    date_num = 0
    for score in raw_files:
        ...
        for release in releases:
            ...
            release_date = release[1].replace('/', '-')
            release_date = datetime.datetime.strptime(release_date, date_str)
            release_content = release[7]
            if release_date <= date_list[date_num] and release_date > date_list[date_num + 1]:
                for n in release_content:
                    front_num += 1
                num += front_num
...
def get_downloads(app):
    result_file = 'month_score/{}_all_month_score_d.csv'.format(app)
    result = open(result_file, 'w', encoding='utf-8', newline='')
    csv_writer = csv.writer(result)
    lines = 0
    # 15个关注点均分
    raw_files = read_csv('month_score/{}_all_month_score_rf.csv'.format(app))
    downloads = pd.read_csv('other_factor/download/{}_download.csv'.format(app),header=None)
    downloads=downloads.drop(0)
    downloads.to_csv('other_factor/download/{}_download.csv'.format(app),header=None,encoding='utf-8',index=False)
    downloads = read_csv('other_factor/download/{}_download.csv'.format(app))
    date_num = 0
    for score in raw_files:
        num = 0
        date_list, raw_reviews = get_month_date_and_labeled_reviews(app)
        for download in downloads:
            release_date = download[0]
            release_date = datetime.datetime.strptime(release_date, date_str)
            release_content = int(download[1])
            if release_date <= date_list[date_num] and release_date > date_list[date_num + 1]:
                num += release_content
                # latest_releases_reviews_list[int(label)].append(labeled_review)
            else:
        ...
        date_num += 1
def get_rank(app):
    result_file = 'month_score/final/{}_all_month_score.csv'.format(app)
    result = open(result_file, 'w', encoding='utf-8', newline='')
    csv_writer = csv.writer(result)
    lines = 0
    # 15个关注点均分
    raw_files = read_csv('month_score/{}_all_month_score_d.csv'.format(app))
    downloads = pd.read_csv('other_factor/rank/{}_rank.csv'.format(app),header=None)
    downloads=downloads.drop(0)
    downloads.to_csv('other_factor/rank/{}_rank.csv'.format(app),header=None,encoding='utf-8',index=False)

    downloads = read_csv('other_factor/rank/{}_rank.csv'.format(app))
    date_num = 0
    for score in raw_files:
        num = 0
        date_list, raw_reviews = get_month_date_and_labeled_reviews(app)
        for download in downloads:
            ...
            if release_date == date_list[date_num]:
                #print(release_date)
                #print(download[1])
                if download[1]=='':
                    release_content = 1500
                else:
                    release_content = download[1]
...
if __name__ == '__main__':
    ...
    app_name_list = ['58job']
    app_name_list= ['didi', 'tongcheng', 'shenzhou', 'douban',
                    'qingteng', 'qqzone', 'uc', 'pengpai',
                    'wangyi', 'tengxunmap', 'weiche',
                    'dida', 'kugou', 'migu', 'lizhi',
                    'meituan','beike', 'zhiyuanhui', 'xigua',
                    'migutv', 'changba', 'baidutext', 'bubei',
                    'hongen']
    # app_name_list =['hongen']
    for app in app_name_list:
        #get_release_date(app)
        get_reviews_num(app)
        get_release_num(app)
        get_release_front_num(app)
        get_downloads(app)
        get_rank(app)
    exit()
