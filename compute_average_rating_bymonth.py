...
if __name__ == '__main__':
    ...
    app_name_list = ['zhitong']
    num=36
    for app_name in app_name_list:
        result_file = 'data/month_score/{}_all_month_score.csv'.format(app_name)
        result = open(result_file, 'w', encoding='utf-8', newline='')
        csv_writer = csv.writer(result)
        lines = 0
        while start_release_no < num:
            # start_release_no从0开始，lines从1开始
            lines += 1
            # chinese_app_name = ['钉钉', '支付宝', '网易云音乐', '腾讯视频']
            all_release_change_average_ratings = []
            release_date_list = []
            # result_file = 'data/release_score/58job.csv'
            # result = open(result_file, 'w', encoding='utf-8', newline='')
            # csv_writer = csv.writer(result)
            rating_list = []
            ...
            release_num = len(release_date_list)
            date_start = release_date_list[start_release_no]
            date_medium = release_date_list[start_release_no + 1]
            # date_end = release_date_list[start_release_no + num_of_releases + 1]
            date_end = release_date_list[start_release_no + num_of_releases + 1]
            ...
            pre_average_ratings = get_average_rating(pre_releases_reviews_list)
            latest_average_ratings = get_average_rating(latest_releases_reviews_list)
            if start_release_no == 0:
                csv_writer.writerow(latest_average_ratings)
                csv_writer.writerow(pre_average_ratings)
            else:
                csv_writer.writerow(pre_average_ratings)
            start_release_no += 1
        csv_writer.writerow(pre_average_ratings)
